...
local function getOptions()
  local profile = getProfileOptions()
  profile.order = 7
  return {
    name = "Glassy",
    handler = C,
    type = "group",
    args = {
      general = getGeneralOptions(),
      editBox = getEditBoxOptions(),
      messages = getMessageOptions(),
      timestamps = getTimestampOptions(),
      combatLog = getCombatLogOptions(),
      compatibility = getCompatibilityOptions(),
      shortcuts = getShortcutOptions(),
      profile = profile,
      about = getAboutOptions(),
    },
  }
end
